...
| 🎞️ **动态回放（规划中）** | 默认保留轻量的交互后动态证据；只要用户明确要求“动态录制”，或检测到动态异常，就会生成 GIF 或短回放 |
...
对于 `jump`、`jitter`、`shake`、`flicker`、`flash`、`blink`、`bounce`、`scroll jump`、`layout shift`、`reflow`、`瞬移`、`抖动`、`跳动`、`闪一下`、`闪烁` 这类动态问题，UI Commander 计划默认保留一小段交互后的 motion evidence。只要用户一开始明确说 `动态录制`、`启动动态录制`，就应直接进入动态录制模式；如果系统判断这是明显的动态异常，或用户明确要求录制这类动态问题，也同样应额外生成 GIF 或短回放。
...
| `帮我看一下这个页面的问题` |
| `点击后页面会跳一下` |
| `提交后有 layout shift / flicker / jitter` |
